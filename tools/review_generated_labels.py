    ...
    def state(self, query: dict[str, str]) -> dict[str, object]:
        ...
        for image_path in self.image_paths():
            ...
            if not include_empty and not boxes:
                continue
            if not matched_boxes and any(
                query.get(name) not in (None, "")
                for name in ("area_lt", "width_lt", "width_gt", "height_lt", "height_gt")
            ):
                continue
            samples.append(
                {
...
def make_handler(app: ReviewApp) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            parsed = urlparse(self.path)
            if parsed.path not in ("/api/mask", "/api/box", "/api/merge"):
                self.send_error(404)
                return
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    main { display: grid; grid-template-columns: minmax(520px, 1fr) minmax(320px, 380px) minmax(280px, 340px); grid-template-rows: auto minmax(0, 1fr); gap: 14px; height: calc(100vh - 32px); max-width: 1800px; margin: 0 auto; overflow: hidden; }
    ...
    .filter-row { display: grid; grid-template-columns: 20px 1fr 118px; gap: 8px; align-items: center; padding: 8px; border: 1px solid #d0d7de; border-radius: 6px; background: #f6f8fa; }
    .filter-row input[type="checkbox"] { justify-self: center; }
    .filter-title { font-size: 13px; font-weight: 600; }
    .filter-help { font-size: 12px; color: #57606a; }
    ...
    .checks { display: flex; flex-wrap: wrap; gap: 10px 16px; margin: 10px 0; }
    .checks label { display: inline-flex; grid-auto-flow: column; align-items: center; gap: 6px; }
  ...
  <aside class="panel">
    <div class="filters">
      <div class="filter-row">
        <input id="areaEnabled" type="checkbox" aria-label="启用面积过小过滤">
        <div><div class="filter-title">面积过小</div><div class="filter-help">找出面积小于阈值的框，单位：像素²</div></div>
    ...
    <div class="row">
      <button id="reload">应用过滤</button>
      <button id="maskMatched">屏蔽当前匹配框</button>
    </div>
    <hr>
...
const ids = ["areaEnabled", "areaLt", "widthLtEnabled", "widthLt", "widthGtEnabled", "widthGt", "heightLtEnabled", "heightLt", "heightGtEnabled", "heightGt", "includeEmpty", "showMasked", "jumpImage"];
const filterIds = ids.filter(id => id !== "jumpImage");
...
function qs() {
  ...
  if (el.includeEmpty.checked) p.set("include_empty", "1");
  if (el.showMasked.checked) p.set("show_masked", "1");
  return p.toString();
}
...
function renderPanel() {
  const sample = current();
  meta.textContent = `${imageIndex + 1}/${state.samples.length} ${sample.image} | ${sample.width}x${sample.height} | boxes=${sample.boxes.length} matched=${sample.matched_count} | total=${state.stats.total_boxes} masked=${state.stats.masked_boxes}`;
  ...
  boxList.innerHTML = sample.boxes.map((box, i) => `
    <div class="box ${i === boxIndex ? "active" : ""} ${selectedBoxes.has(i) ? "active" : ""} ${box.masked ? "masked" : ""} ${box.matched ? "matched" : ""}" data-i="${i}">
      ...
      <br>x=${box.x} y=${box.y} w=${box.width} h=${box.height} area=${box.area} ratio=${box.aspect}
    </div>`).join("");
...
canvas.addEventListener("pointerup", event => {
  ...
  if (sample && box) commitBoxEdit(sample, box, previousRect);
});

canvas.addEventListener("pointercancel", () => {
  const box = currentBox();
  if (drag && box) Object.assign(box, drag.previousRect);
  drag = null;
  draw();
});
...
filterIds.forEach(id => el[id].addEventListener("change", () => loadState()));
[
  ["areaEnabled", "areaLt"],
